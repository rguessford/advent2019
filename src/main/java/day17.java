...
public class day17 {
    ...
    public static void main(String[] args) {
        ...
        long[] memory;
        try {
            reader = new BufferedReader(new FileReader("src/main/res/day17"));
            memory = Arrays.stream(reader.readLine().split(",")).mapToLong(Long::parseLong).toArray();
            memory = Arrays.copyOf(memory, memory.length +4000);
            char[][] screen = new char[41][77];
            IntcodeCamera camera = new IntcodeCamera(memory);
            camera.render();
    ...
    public static class IntcodeCamera {
        long[] memory;
        long outputRegister;
        ...
        public int render() {
            ...
            int y = 0;
            int inputdex = 0;
            String inputs = "A,B,A,C,B,C,A,B,A,C`R,6,L,10,R,8,R,8`R,12,L,8,L,10`R,12,L,10,R,6,L,10`n`";
            while (true) {
                ...
                if (instruction[OPCODE] == 1) {
                    ...
                    //input
                    char in = inputs.charAt(inputdex);
                    inputdex++;
                    if(in=='`') in = 10;
                    if (instruction[PARAMETER_MODE_1] == POSITION_MODE) {
